...
@dp.message_handler(commands=['adminDan'])
...
async def handle_contact(callback_query: types.CallbackQuery):
    new_photo = open('bot/contact.jpg', 'rb')  # Укажите путь к фотографии
...
async def process_callback_book(callback_query: types.CallbackQuery, state: FSMContext):
  ...
  # Replace with your admin's ID
    ...
    await bot.send_message(chat_id=channel_id, text=admin_message, parse_mode='Markdown')
        # Answer the callback query
    try:
